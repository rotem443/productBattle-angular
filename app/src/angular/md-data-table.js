    ...
    function mdtAlternateHeadersDirective(){
        return {
            ...
            link: function($scope){
                ...
                $scope.createSelectedBattels = createSelectedBattels;

                function deleteSelectedRows(){
                    $scope.tableDataStorageService.deleteSelectedRows().then(function(results) {

                        var deletedRows = results;

                        $scope.deleteRowCallback({rows: deletedRows});
                    });
                };

                function createSelectedBattels(ev){
                    $scope.createBattleCallback({rows : $scope.tableDataStorageService.getListSelectedItems(),
                                                 event : ev})
                };
            }
        };
...
(function(){
    ...
    function mdtTableDirective(TableDataStorageFactory, mdtPaginationHelperFactory, mdtAjaxPaginationHelperFactory){
        return {
            ...
            link: function($scope, element, attrs, ctrl, transclude, $rootScope){
...
(function(){
    ...
    function TableDataStorageFactory($log, $http, $q){
        ...
        TableDataStorageService.prototype.deleteSelectedRows = function(){
            var arrReturnResult = $q.defer();
            var localStorage = this.storage;

            var getDeleteItems = function(bDelete) {

                var selectedRows = [];

                _.each(localStorage, function (rowData) {
                    if (rowData.optionList.selected && rowData.optionList.deleted === false) {
                        ...
                        rowData.optionList.deleted = bDelete;
                    }
                });
        ...
        TableDataStorageService.prototype.getListSelectedItems = function() {
            var selectedRows = [];
            ...
            _.each(this.storage, function(rowData){
                if(rowData.optionList.selected){
                    ...
                    if(rowData.rowId){
                        selectedRows.push({'id' : rowData.rowId,
                                           'name' : rowData.data[0]
                        });
        ...
        return {
            getInstance: function(){
                ...
                if (this.myTable == undefined) {
                 this.myTable =  new TableDataStorageService();
                }
